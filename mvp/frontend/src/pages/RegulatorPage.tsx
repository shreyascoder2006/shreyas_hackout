  ...
  return (
    <main className="flex flex-1 flex-col gap-3 overflow-hidden p-4">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        ...
        <Kpi label="Avg deviation" value={`+${state.avgDeviationPct}%`} sub="vs sub-sector benchmark" />
              ...
              {state.clusters.map((c) => (
                <button key={c.cluster.id} onClick={() => selectCluster(c.cluster.id)} className="flex items-center justify-between border-b border-[color:var(--color-border)] px-4 py-2.5 text-left hover:bg-[color:var(--color-panel-2)]/60">
                  <div>
                    ...
                    <div className="text-[11px] text-[color:var(--color-muted)]">{c.factories} units · {c.hotspotProcesses} hotspot processes · +{c.avgDeviationPct}%</div>
                  </div>
                      ...
                      <tr key={s.sector} className="border-t border-[color:var(--color-border)]">
                        ...
                        {s.suppressed ? (
                          ...
                          <>
                            ...
                            <td className="text-right">+{s.avgDeviationPct}%</td>
